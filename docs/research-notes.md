> **这是第一轮的调研交接文档（2026-08），保留原文与原章节号。**
> 它记录的是动手之前的调查与本机实测，代码注释与 `DESIGN.md` 里的 §x.y 引用都指向这里。
> **凡与 `DESIGN.md` 冲突之处，以 `DESIGN.md` 为准** —— 那份文档是在这份之上做的修正。
> 面向使用者的说明见仓库根目录的 `README.md`。
